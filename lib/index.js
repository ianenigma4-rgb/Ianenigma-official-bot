...
// ── PERFORMANCE: in-memory cache so group data is never re-read from disk
//    on every single message. Writes are async (non-blocking).
let _userGroupDataCache = null;
const DATA_PATH = path.join(process.env.BOT_DATA_DIR || path.join(__dirname, '../data'), 'userGroupData.json');
const DEFAULT_DATA = () => ({ antibadword: {}, antilink: {}, welcome: {}, goodbye: {}, chatbot: {}, warnings: {}, sudo: [] });

// Function to load user and group data from JSON file
function loadUserGroupData() {
    if (_userGroupDataCache !== null) return _userGroupDataCache;
    try {
        if (!fs.existsSync(DATA_PATH)) {
            _userGroupDataCache = DEFAULT_DATA();
            fs.writeFileSync(DATA_PATH, JSON.stringify(_userGroupDataCache, null, 2));
            return _userGroupDataCache;
        }
        _userGroupDataCache = JSON.parse(fs.readFileSync(DATA_PATH, 'utf8'));
        return _userGroupDataCache;
    } catch (error) {
...
// Function to save user and group data — updates cache immediately, writes async
function saveUserGroupData(data) {
    _userGroupDataCache = data;
    const dir = path.dirname(DATA_PATH);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.promises.writeFile(DATA_PATH, JSON.stringify(data, null, 2)).catch(error => {
