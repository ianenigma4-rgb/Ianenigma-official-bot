...
const STATE_FILE = path.join(process.env.BOT_DATA_DIR || path.join(__dirname, '../data'), 'dailyScheduler.json');
...
const GOODNIGHT_MESSAGES = [
    ...
    "Sleep well, IAN ENIGMA. The bot will keep watch tonight. 🦇",
...
async function runDailyScheduler(sock) {
    try {
        ...
        if (hour === 7 && !state[today].morning) {
            ...
            await sock.sendMessage(ownerJid, {
                text: `🌅 *GOOD MORNING, IAN ENIGMA!* 🌅\n` +
                      ...
                      `${fact}\n` +
                      `━━━━━━━━━━━━━━━━━━━━━━━\n` +
                      `🦇 *IAN ENIGMA MD BOT* is online and protecting Gotham.\n` +
        ...
        if (hour === 12 && !state[today].goodday) {
            ...
            await sock.sendMessage(ownerJid, {
                text: `☀️ *GOOD AFTERNOON, IAN ENIGMA!*\n\n` +
                      `🕐 ${time} | ${loc.flag} ${loc.city}, ${loc.country}\n\n` +
                      `${msg}\n\n` +
                      `_🦇 IAN ENIGMA MD BOT standing by._`
            });
        }
