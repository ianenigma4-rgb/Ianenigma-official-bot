...
async function isAdmin(sock, chatId, senderId) {
    try {
        ...
        const senderLidNumeric = senderId.includes(':') ? senderId.split(':')[0].split('@')[0] : senderIdWithoutSuffix;
        ...
        const isSenderAdmin = participants.some(p => {
            ...
            // Extract numeric part from participant LID (remove session identifier like :4)
            const pLidNumeric = pLid.includes(':') ? pLid.split(':')[0] : pLid;
            ...
            const senderMatches = (
                ...
                senderIdWithoutSuffix === pLid || // Sender ID vs participant LID (with suffix, legacy check)
                senderLidNumeric === pLidNumeric || // Sender LID numeric vs participant LID numeric (KEY FIX)
                senderLidNumeric === pLid || // Sender LID numeric vs participant LID raw
                senderLidNumeric === pId || // Sender LID numeric vs participant ID
                senderLidNumeric === pPhoneNumber // Sender LID numeric vs participant phone
            );
