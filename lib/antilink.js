		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `🦇 Link deleted. Links are not allowed here, @${sender.split('@')[0]}.`,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `🦇 @${sender.split('@')[0]} has been removed from the group for sending links.`,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `🦇 @${sender.split('@')[0]} has been removed from the group after ${WARN_COUNT} warnings.`,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
