...
function getResult(total) {
    ...
    if (pct <= 20) return { level: '✅ Minimal', msg: 'You show very few ADHD-like symptoms. Your focus and impulse control appear strong.', advice: 'Keep up your routine and self-care habits!' };
    if (pct <= 40) return { level: '🟡 Mild', msg: 'You show some mild symptoms that are common in everyday life. Not necessarily ADHD.', advice: 'Try structured to-do lists and regular breaks (Pomodoro technique).' };
    if (pct <= 60) return { level: '🟠 Moderate', msg: 'You show moderate ADHD-like symptoms. These may be affecting your daily life.', advice: 'Consider speaking to a professional — lifestyle changes can help significantly.' };
    if (pct <= 80) return { level: '🔴 High', msg: 'You show significant ADHD-like symptoms. These may be causing real difficulties.', advice: '⚕️ Strongly consider a professional evaluation from a psychiatrist or psychologist.' };
    return { level: '🚨 Very High', msg: 'You show very high ADHD-like symptoms across multiple areas.', advice: '⚕️ Please consult a licensed mental health professional. This test is not a diagnosis but your responses suggest professional support would be very beneficial.' };
}
...
async function adhdtestCommand(sock, chatId, message, rawText, senderId) {
    try {
        ...
        if (/^[1-5]$/.test(arg)) {
            ...
            if (!session) {
                return sock.sendMessage(chatId, { text: '❌ No active test. Start one with *.adhdtest start*' }, { quoted: message });
            }
        ...
        console.error('Error in adhdtest command:', error);
