...
async function backupCommand(sock, chatId, senderId, message, isOwnerOrSudoCheck) {
    try {
        if (!isOwnerOrSudoCheck && !message.key.fromMe) {
            return sock.sendMessage(chatId, { text: '❌ Owner/sudo only.', ...channelInfo }, { quoted: message });
        }
        ...
        await sock.sendMessage(chatId, {
            ...
            fileName: 'ianenigma_backup_' + new Date().toISOString().split('T')[0] + '.zip',
            caption: '✅ Backup complete!\n📦 Size: ' + (stat.size / 1024).toFixed(1) + 'KB\n📅 Date: ' + new Date().toLocaleString()
        }, { quoted: message });
        fs.unlink(outFile, () => {});
    } catch (error) {
        console.error('Error in backup command:', error);
        await sock.sendMessage(chatId, { text: '❌ Backup failed. Please try again later.' }, { quoted: message });
    }
}
